...
export function isPointInPolygon(
  ...
  const x = point.longitud; // /ngitud como x
  const y = point.latitud;  // // latitud como ynst n = polygon.length;
  let inside = false;
  ...
  for (let i = 0; i <= n; i++) {
    ...
    // // Verificamos si elnto está sobre un vértice o borde
    if ((x === p1x && y === p1y) || (x === p2x && y === p2y)) {
      return true;
    }

    // // Verificamos si están un borde horizontal
    if (p1y === p2y && y === p1y) {
