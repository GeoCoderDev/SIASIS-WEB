...
// // 🏷️num para los tipos de identificadores
...
exportnst TiposIdentificadoresTextos: Record<TiposIdentificadores, string> =
  {
...
/**
* 🔍 Función para extraer el tipo de identificador basándose en lo que viene después del guión Si el tipo no existe o es inválido, cierra la sesión automáticamente @param identificador - Identificador en formato {identificador}-{tipo} @returns TiposIdentificadores - El tipo de identificador válido @example // // Ejemplos de uso: extraerTipoDeIntificador("12345678-1")    // // → TiposIntificadores.DNI (1) extraerTipoDeIdentificador("A123456-2")     // // → TiposIntificadores.CARNET_EXTRANJERIA (2) extraerTipoDeIdentificador("ESC123-3")      // // → TiposIntificadores.CODIGO_ESCUELA (3) extraerTipoDeIdentificador("12345678-5")    // // → Cierra sesn (tipo inválido) extraerTipoDeIdentificador("12345678")      // // → TiposIntificadores.DNI (compatibilidad)
*/
export function extraerTipoDeIdentificador(
  identificador: string
): TiposIdentificadores {
  // // 🧹 Limpiar el intificador de espacios en blanco
  const identificadorLimpio = identificador.trim();

  // // 🔍 Caso 1: DNI de 8 dígitosn guión (compatibilidad hacia atrás)
  if (/^\d{8}$/.test(identificadorLimpio)) {
    return TiposIdentificadores.DNI;
  }

  // // 🔍 Caso 2: Formaton guión {identificador}-{tipo}
  const partesIdentificador = identificadorLimpio.split("-");

  // // ❌ Sno tiene guión o tiene formato incorrecto, asumir DNI por compatibilidad
  if (partesIdentificador.length !== 2) {
    return TiposIdentificadores.DNI;
  }

  // // 📊 Extraer el tipnumérico de la parte después del guión
  const tipoNumerico = parseInt(partesIdentificador[1], 10);

  // // ✅ Verificar que el tipo extraído existen el enum
  ...
  // // 🚨 TIPO INVÁLIDO: Cerrar sesn por seguridad
  console.error(
  ...
  // // Estento nunca se alcanzará porque logout redirige, pero TypeScript lo requiere
