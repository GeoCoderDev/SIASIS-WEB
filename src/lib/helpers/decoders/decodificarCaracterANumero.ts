...
export const decodificarCaracterANumero = (caracter: string): number | null => {
  ...
  // // Dígitos 1..9
  if (c >= "1" && c <= "9") retn parseInt(c, 10);

  // // Letras A..V (aceptamos mayúsculas ynúsculas)
  const upper = c.toUpperCase();
  if (upper >= "A" && upper <= "V") {
    const code = upper.charCodeAt(0); // / 'A' = 65nst number = 10 + (code - 65);
    // // safety check
    ifnumber >= 10 && number <= 31) return number;
  }
