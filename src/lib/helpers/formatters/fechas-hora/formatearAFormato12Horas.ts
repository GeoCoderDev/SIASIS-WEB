...
export function formatearISOaFormato12Horas(
  ...
  try {
    // // Crear objeto Date a partir del timestampnst fecha = new Date(timestamp);

    // // Verificar si la fecha es válida
    if (isNaN(fecha.getTime())) {
      retn "Formato de fecha inválido";
    }

    // // Obner horas y minutos dependiendo si ya es UTC o no
    ...
    // //nvertir a formato 12 horas
    horas = horas % 12;
    horas = horas ? horas : 12; // / Si es 0, mostrar como 12

    // Formatearnutos con ceros a la izquierda si es necesario
    const minutosFormateados = minutos < 10 ? `0${minutos}` : minutos;

    // //nstruir la cadena de resultado
...
// //nción alternativa que incluye los segundos si son diferentes de 00
export function convertirAFormato12Horas(
  ...
  // // Soloncluir segundos si son diferentes de 00
  if (!withSeconds) {
