...
export class IndexedDBConnection {
  ...
  // // Propiedad estática que senicializa de forma inteligente
  private static _rol: RolesSistema | null = null;
  private static _PostfixIDB: string | null = null;

  // // Usamos la variable dentorno para la versión
  ...
  private constructor() {
    // //nstructor privado para patrón Singleton
...
*/
  public static get rol(): RolesSistema {
    // // Sno está seteado, intentar cargar desde localStorage
...
*/
  public static set rol(newRol: RolesSistema) {
    ...
    // // Guardarn localStorage si estamos en el cliente
...
*/
  private static loadRolFromStorage(): RolesSistema {
    // // Verificar si estamosn el cliente
    if (typeof window !== "undefined" && window.localStorage) {
    ...
    // // Valor por defecto sno hay nada en localStorage o no es válido
...
*/
  public async changeRole(newRole: RolesSistema): Promise<void> {
    ...
    // // Si es el mismo rolno hacer nada
    if (currentRole === newRole) return;

    // // Cerrar lanexión actual
    this.close();

    // // Cambiar el rol (esto automáticante actualiza localStorage)
    IndexedDBConnection.rol = newRole;

    // // Rnicializar con la nueva base de datos
    await this.init();
  }
...
*/
  public async init(): Promise<IDBDatabase> {
    // // Verificar que estamosn el cliente
    ...
    this.initPromise = new Promise((resolve, reject) => {
      // // Al abrirn una versión superior, IndexedDB automáticamente
      // // disparanupgradeneeded y gestiona la migración
      ...
      request.onupgradeneeded = (event) => {
        ...
        // // Si hay stores exisntes que ya no necesitamos, los eliminamos
...
*/
  private configureDatabase(db: IDBDatabase): void {
    // // Crear los object stores y susndices
...
*/
  private getVersionNumber(versionString: string): number {
    // // Elinar cualquier sufijo (como -alpha, -beta, etc.)
    const cleanVersion = versionString.split("-")[0];

    // // Dividir porntos y convertir a un número entero
    // // Por ejemplo: "1.2.3" -> 1 * 10000 + 2 * 100 + 3 = 10203nst parts = cleanVersion.split(".");
