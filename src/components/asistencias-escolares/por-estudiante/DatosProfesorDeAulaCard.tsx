...
const DatosProfesorDeAulaCard = ({
  ...
  const abrirWhatsApp = (celular: string) => {
    ...
    const mensaje = encodeURIComponent(
      `Hello ${obtenerTitulo()} ${
        profesor?.Nombres
      }, I am a parent from I.E. 20935 Asunción 8 school`
    );
  ...
  const obtenerTitulo = () => {
    if (!profesor) return "";
    return profesor.Genero === "F" ? "Teacher" : "Professor";
  };
  ...
  // If no teacher assigned (confirmed)
  if (!profesor || !aula) {
    return (
      <div className="bg-white rounded-lg shadow-md p-3">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-400">Classroom without teacher</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4">
      {/* Header with photo and name - more spacious */}
      <div className="flex  gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-gray-800 leading-tight mb-1">
            {obtenerTitulo()}{" "}
            {aula.Nivel === NivelEducativo.PRIMARIA ? "Primary" : "Secondary"}{" "}
            - {aula.Grado}
            {aula.Seccion}:
          </h3>
      ...
      {/* Contact information with more breathing room */}
      <div className="space-y-3">
        {profesor.Celular ? (
          <>
            ...
            {/* WhatsApp button with more padding */}
            <button
          ...
          /* Message when no cell phone - more spacious */
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-center">
            ...
            <p className="text-xs text-gray-500">
              Not available for direct contact
            </p>
          </div>
        )}
      </div>
