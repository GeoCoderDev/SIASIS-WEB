...
// //nstante para el offset de tiempo (para pruebas)
// / Modificar estos valores para cambiar el offset aplicado a la hora del servidor
exportnst TIME_OFFSET = {
  days: OFFSET_DIAS_ADICIONALES_SIU01, // / Días adicnales para el offset
  hours: OFFSET_HORAS_ADICIONALES_SIU01, // / Agregar propiedad 'hours'n un valor predeterminado
...
//nterfaces para datos de tiempo formateados y utilidades
export interface FormatosHora {
  ...
  // // Nuevos formatos
  fechaLegible: stng; // / Ejemplo: "nes, 15 de Enero de 2024"
  fechaNumericaCorta: string; // / Ejemplo: "15/01/2024"
  horaAmPm: stng; // / Ejemplo: "10:30 AM"
}

exportnterface UtilidadesTiempo {
  hora: number;
  minutos: number;
  segundos: number;
  esDiaEscolar: boolean;
  diaSemana: string; // / Nombre del día (nes, Martes, etc)
  diaSemanaCorto: string; // / Abreviatura (n, Mar, etc)
  diaSemanaIndice: number; // / 0-6 (0 = Dongo, 6 = Sábado)
  diaMes: number; // / 1-31
  mes: stng; // / Nombre del mesnero, Febrero, etc)
  mesCorto: string; // / Abreviaturane, Feb, etc)
  mesIndice: number; // / 0-11 (0 =nero, 11 = Diciembre)
  año: number; // / Año completo (ej: 2024)
  dianMes: number; // / Número de díasn el mes actual
  esFinDeSemana: boolean; // / true si es Sábado o Dongo
  trimestre: number; // / 1-4 (trimestre del año)
  senaDelAño: number; // / 1-53
  diaDelAñonumber; // / 1-366
  esHoy: booln; // / true si la fecha es hoy (n considerar la hora)
  timestamp: number; // / timestampn milisegundos
}

// //nterfaz para la fecha y hora actual con datos formateados
...
export const obtenerSemanaDelAño = (fecha: Date): number => {
  // // Crearna copia de la fecha para no modificar la original
  const fechaCopia = new Date(fecha);

  // // Obner el primer día del año
  const primerDiaAño = new Date(fecha.getFullYear(), 0, 1);

  // // Ajustar al primer día de la sena (domingo)
  const diaSemana = primerDiaAño.getDay();
  primerDiaAño.setDate(primerDiaAño.getDate() - diaSemana);

  // // Calcular días tnscurridos desde el inicio del año
...
// // Tnk para obtener la hora del servidor
export const fetchFechaHoraActual = createAsyncThunk(
  ...
  async (timezone: string = ZONA_HORARIA_LOCAL, { rejectWithValue }) => {
    try {
      ...
      // // Crearna fecha usando el timestamp ya ajustado a la zona horaria
      const fechaLocal = new Date(data.serverTime);

      // // Aplicamos el offset si está habilitado
      if (TIME_OFFSETnabled) {
...
// //nción auxiliar para actualizar formatos y utilidades
const actualizarFormatosYUtilidades = (state: FechaHoraActualRealState) => {
  ...
  // // Actualizar formatosn especificar timeZone para evitar doble ajuste
  state.formateada = {
    ...
    // // Nuevos formatos
    fechaLegible: `${
      diasSenaTextos[fechaHoraDate.getDay() as DiasSemana]
  ...
  // // Obner datos adicionales de la fecha
  ...
  // // Calcular valores derivadosnst diasEnMes = obtenerDiasEnMes(año, mesIndice);
  ...
  state.utilidades = {
    hora,nutos,
    segundos,
    esDiaEscolar: diaSemanaIndice >= 1 && diaSemanaIndice <= 5, // /nes a Viernes

    // // Nuevos campos
    diaSena: diasSemanaTextos[diaSemanaIndice],
...
const fechaHoraActualRealSlice = createSlice({
  ...
  reducers: {
    ...
    updateFechaHoraActual: (state) => {
      if (state.fechaHora) {
        // // Parseamos la fecha actualnst fechaActual = new Date(state.fechaHora);

        // // Añadimosn segundo para que el tiempo avance
        fechaActual.setSeconds(fechaActual.getSeconds() + 1);

        // // Actualizamos el estadon la nueva fecha
    ...
    setTimene: (state, action: PayloadAction<ReduxPayload<string>>) => {
      state.timezone = action.payload.value;
      // // Actualizamos formatos y utilidadesn la nueva zona horaria
      actualizarFormatosYUtilidades(state);
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchFechaHoraActual.rejected, (state, actn) => {
